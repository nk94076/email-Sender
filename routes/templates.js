...
router.post('/', (req, res) => {
  ...
  const result = db
    .prepare('INSERT INTO templates (name, subject, html_body) VALUES (?, ?, ?)')
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(
    "UPDATE templates SET name = ?, subject = ?, html_body = ?, updated_at = datetime('now') WHERE id = ?"
  ).run(name ?? existing.name, subject ?? existing.subject, html_body ?? existing.html_body, req.params.id);
  res.json({ ok: true });
});
