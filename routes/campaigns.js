...
router.post('/', (req, res) => {
  const { template_id, list_id, subject } = req.body;
  ...
  const result = db
    .prepare('INSERT INTO campaigns (template_id, list_id, subject, total) VALUES (?, ?, ?, ?)')
    .run(template_id, list_id, subject || template.subject, recipientCount);
